...
test("recipe entry shows unknown, surplus and disabled inventory consistently", async ({ page }) => {
  ...
  await expect(page.getByLabel("Unknown owned, 2 required; unknown remaining", {exact: true})).toBeVisible();
  await page.goto("./#/kh1fm/synthesis/materials");
  ...
  await page.goto(`./#/kh1fm/entry/${recipe.entryId}`);
  await expect(page.getByLabel("8 owned, 2 required; 0 remaining", {exact: true})).toBeVisible();
  await page.goto("./#/kh1fm/synthesis/materials");
...
test("reference, challenge and synthesis filters survive entry visits and reloads", async ({ page }) => {
  ...
  await page.locator(".entry-row h3 a").first().click();
  await page.getByRole("link", {name: "Back to reference", exact: true}).click();
  ...
  await page.locator(".recipe-card h2 a").click();
  await page.getByRole("link", {name: "Back to synthesis", exact: true}).click();
  await page.reload();
